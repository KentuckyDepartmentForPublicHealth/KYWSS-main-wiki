---
title: "NWSS WVAL Calc README"
---

# Wastewater Virus Activity Level (WVAL) Calculation Repo README

This repository calculates weekly WVALs at the site level and then creates files aggregated to the state, regional, and national levels to support the new NWSS dashboards on the [new site](https://www.cdc.gov/nwss/index.html).

## WVAL Calculation Script

### Filtering and Joining Geography Columns

The script joins in the following geographical columns: - HHS Region - US Census region. This uses filters found in `src/myproject/utils/filters.py`

The following filters are applied: - pcr target is sars-cov-2 - certain tribal and institutional data is excluded, consistent with CDT data practices and upstream sites are excluded - rows with null site_ids are excluded - rows which are not MLM-adjusted are excluded - the source data comes from the all final MLM analytics dataset which consists of daily values averaged based on MLM
...
-   Through several steps, a `site_id_with_pcr_source_mlm_norm` column is created to ensure the WVALs are only calculated using self-consistent data.
    -   Uses the `site_id`, `pcr_target`, `source`, `major_lab_method`, and `norm_method` columns.
...
``` python
  all_final = all_final.withColumn(
        "pcr_target_flowpop_ln",
        F.log("pcr_target_flowpop_lin")
    ).withColumn(
        "pcr_target_mic_ln",
        F.log("pcr_target_mic_lin")
    )
```
...
WVALs are calculated on a `site_id_with_pcr_source_mlm_norm` (hereafter, site-combo) basis for each sample. - A WVAL is only calculated for a sample if its site-combo has been sampling \>= 42 days - WVAL is preferentially calculated using **flowpop-normalized** concentration, if available, and microbial-normalized concentration otherwise. - The number of standard deviations (SDs) above baseline is first calculated using natural log transformed concentration as: - (sample concentration - baseline concentration) / standard deviation - see details on baseline calculation in subsection - Finally, the number of SDs above is exponentiated by e (back transformed) to obtain the WVAL in linear units which we have found to be more interpretable for graphing
...
``` python
  # Calculate ww_index_lin as the number of standard deviations from the 10th percentile baseline
    ...
    all_final = all_final.withColumn(
        "ww_index_normed_ln",
        F.when(
            F.col("days_site_online") < 42, None
        ).when(
...
#### Backfilling WVALs after 6 weeks

For the first 6 weeks a site-combo is online, its WVAL will not be calculated. However, once a site has been online \> 6 weeks (42 days), WVALs will be calculated for all of its samples and will be included in historical trends calculation for the site as well as the associated state and region.
...
However, the time window / frequency of updating the baseline depends on how long a site-combo has been sampling: - For the first 6 months a site-combo is online, the baseline is updated each time there is a new sample - At the 6 month point, baseline is held constant at the last calculated value until the next update check point - After the 6 month point, the baseline is updated for a site-combo semi-yearly on January 1 and July 1.

After the preceding row to row calculations, the final step is to retrieve the last calculated baseline and standard deviation for a site combo. We then ensure that these "last values" are used to calculate the WVAL for all samples for a given site-combo.
...
1.  The `temp_baseline` is calculated for rows that match the update requirements mentioned above: `python     # First, calculate the baseline where needed (first 6 months or nearest to Jan 1/Jul 1)         all_final = all_final.withColumn(             "temp_baseline",             F.when(                 (F.col("days_since_first_sample") <= 182) |                 (F.col("days_to_nearest_target") == F.col("min_days_to_target")),                 F.percentile_approx("metric_column_norm_ln", 0.1).over(percentile_range_window)             ).otherwise(F.lit(None))         )`
...
3.  To get the `last_baseline_ln`, the dataset is aggregated by site-combo and the last baseline value calculated is retrieved. `python     # BASELINE AND STD BACK APPLY ###         # get last calculated baseline and std to apply to all ww index calcs for combo         last_vals = all_final.groupby("site_id_with_pcr_source_mlm_norm").agg(             F.last("baseline_norm_ln").alias("last_baseline_ln"),             F.last("std_norm_ln").alias("last_std_ln"),         )`
